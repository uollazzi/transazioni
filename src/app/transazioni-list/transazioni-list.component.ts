...
export class TransazioniListComponent {
  ...
  transazioneSelezionata(transazione: Transazione) {
    this.msg = transazione.business;
  }

  eliminaTransazione(transazione: Transazione) {
    let indice = this.transazioni.indexOf(transazione);

    if (indice > -1) {
      this.transazioni.splice(indice, 1);
    }
